...
// Support Code
// ---------------------------------------------------------------------------------------------------------------------
const MIGRATION_STEPS: ((data : any) => Promise<any>)[] = [ // Using any's isn't perfect but will do for now
...
export const CURRENT_VERSION = MIGRATION_STEPS.length + 1

// ---------------------------------------------------------------------------------------------------------------------
// Code
// ---------------------------------------------------------------------------------------------------------------------
export async function Migrate(data:any):Promise<IPluginSettings|null>   {
	...
	if (version === -1){
		...
		await plugin.app.vault.adapter.copy(
			`${plugin.manifest.dir}/data.json`,
			`${plugin.manifest.dir}/data_backup.json`
		);
