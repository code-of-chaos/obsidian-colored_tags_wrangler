				...
				folderUUID => {
					...
					return all_tags
						...
						.map(({tag_name, color, background_color, background_opacity}) => {
							...
							.nav-folder.alx-folder-with-note:has(> [data-path="${folder_path}"]) > .nav-folder-title > .nav-folder-title-content,
							...
							.nav-folder:has(> [data-path="${folder_path}"]){
								background-color: rgba(${background_color.r}, ${background_color.g}, ${background_color.b},  ${background_opacity}) ${important};								
